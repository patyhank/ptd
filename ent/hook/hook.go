...
// The PostInfoFunc type is an adapter to allow the use of ordinary
// function as PostInfo mutator.
type PostInfoFunc func(context.Context, *ent.PostInfoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PostInfoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PostInfoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PostInfoMutation", m)
}
