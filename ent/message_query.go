...
func (mq *MessageQuery) QueryAuthor() *AuthorQuery {
	query := (&AuthorClient{config: mq.config}).Query()
...
// QueryParentPost chains the current query on the "parent_post" edge.
func (mq *MessageQuery) QueryParentPost() *PostInfoQuery {
	query := (&PostInfoClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withAuthor:     mq.withAuthor.Clone(),
		withParentPost: mq.withParentPost.Clone(),
...
func (mq *MessageQuery) WithAuthor(opts ...func(*AuthorQuery)) *MessageQuery {
	query := (&AuthorClient{config: mq.config}).Query()
...
// WithParentPost tells the query-builder to eager-load the nodes that are connected to
// the "parent_post" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MessageQuery) WithParentPost(opts ...func(*PostInfoQuery)) *MessageQuery {
	query := (&PostInfoClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withParentPost = query
	return mq
}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	var (
		...
		loadedTypes = [2]bool{
			mq.withAuthor != nil,
			mq.withParentPost != nil,
		}
	)
	if mq.withAuthor != nil || mq.withParentPost != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withAuthor; query != nil {
		if err := mq.loadAuthor(ctx, query, nodes, nil,
			func(n *Message, e *Author) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withParentPost; query != nil {
		if err := mq.loadParentPost(ctx, query, nodes, nil,
			func(n *Message, e *PostInfo) { n.Edges.ParentPost = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadAuthor(ctx context.Context, query *AuthorQuery, nodes []*Message, init func(*Message), assign func(*Message, *Author)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].message_author == nil {
			continue
		}
		fk := *nodes[i].message_author
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "message_author" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) loadParentPost(ctx context.Context, query *PostInfoQuery, nodes []*Message, init func(*Message), assign func(*Message, *PostInfo)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].post_info_messages == nil {
			continue
		}
		fk := *nodes[i].post_info_messages
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_info_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
