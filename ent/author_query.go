...
func (aq *AuthorQuery) QueryMessages() *MessageQuery {
...
func (aq *AuthorQuery) WithMessages(opts ...func(*MessageQuery)) *AuthorQuery {
	query := (&MessageClient{config: aq.config}).Query()
...
func (aq *AuthorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Author, error) {
	...
	if query := aq.withMessages; query != nil {
		if err := aq.loadMessages(ctx, query, nodes,
			func(n *Author) { n.Edges.Messages = []*Message{} },
			func(n *Author, e *Message) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthorQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Author, init func(*Author), assign func(*Author, *Message)) error {
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(author.MessagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.message_author
		if fk == nil {
			return fmt.Errorf(`foreign-key "message_author" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "message_author" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
